	...
	return Promise.all([
		...
		settings.minFree > 0 && Promise.resolve()
			...
			.then(data => data.freePercent <= settings.minFree
				? {diskSpace: `FAIL: Only ${data.freePercent}% disk remaining - ${readable.fileSize(data.used)} / ${readable.fileSize(data.size)} @ ${data.usePercent}% used for mount point ${data.mounted}`}
				: {diskSpace: `OK: ${readable.fileSize(data.used)} / ${readable.fileSize(data.size)} @ ${data.freePercent}% free for mount point ${data.mounted}`}
			),
		// }}}
		// Check writable {{{
		settings.writable && Promise.resolve()
			...
			.then(tempPath =>
				fs.writeFile(tempPath, `Write test generated by Sanity on ${(new Date).toISOString()}`)
					...
					.then(()=> ({writeable: 'OK: Mount point is writable'}))
