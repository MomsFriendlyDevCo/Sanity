...
export default class SanityModule {
	...
	*/
	...
	/**
	* A timestring() compatible frequency (or time in milliseconds) on how often this module should be allowed to run
	* If set to false the module will always be available
	*
	...
	*/
	...
	/**
	* Try to isolate the context so that even out-of-context or top-level throws get captured
	*
	* @type {Boolean}
	*/
	isolate = false;
	...
	*
	* @type {Function}
	* @param {Sanity} sanity The parent sanity instance
	*/
	handler() {
	}
	...
	/**
	* Handler wrapper function including isolation
	*
	* @type {Function}
	*/
	_handler(...args) {
		if (this.isolate) {
			return new Promise((resolve, reject) => {
				...
				domain.run(()=> this.handler.call(this, ...args))
					.then(()=> resolve())
