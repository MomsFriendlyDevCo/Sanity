...
export class Sanity {
	...
	*/
	...
	/**
	* Color pallete to use for various highlighting
	*
	* @type {Boolean|Object} Either boolean false (to disable) or an object of color lookups
	*/
	colors = {
		module: 'blue',
	};
	...
	*
	* @param {String} type The type to look up from `colors`
	...
	*/
	colorize(type, text) {
		return this.colors === false ? text // Colors are disabled anyway
			: this.colors[type] ? chalk[this.colors[type]](text) // Color exists
			: text;
	}
				...
				Object.values(this.modules)
					...
					.map(module => {
						...
						return domain.run(()=> Promise.resolve()
							...
							.then(result => {
								// Tidy up result {{{
								if (Array.isArray(result) && result.length == 1) { // Collapse single array outputs
									result = result[0];
