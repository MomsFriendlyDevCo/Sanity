...
export class Sanity {
	...
	* @param {Object} [options.cache] Initial pre-setup cache instance, if any
	...
	*/
	loadEnv(options) {
		...
		return Promise.resolve()
			...
			.then(()=> {
				if (this.cache) { // Instance already has a cache adapter
					this.log(5, 'Using pre-supplied cache against instance');
					return; // Something already present
				} else if (settings.cache) { // Use provided cache adapter from settings
					this.log(5, 'Using pre-supplied cache - via loadEnv() settings');
					if (typeof settings.cache?.get != 'function') throw new Error('Sanity.cache should be a valid Cache adapter instance');
	...
	*/
	require(path) {
		return Promise.resolve()
			.then(()=> { // Sanity checks
				if (typeof path != 'string') throw new Error('require(path) expects a single file path');
			})
			.then(()=> import(path))
	...
	*
	* @param {Object} [options] Additional options to mutate behaviour
	* @param {String} [options.sort=['FAIL-ERRO-TIME-WARN', 'id']] How to sort output modules. Any uppercase value sorts by those fields seperated by hypens, other fields are sorted by that report key
	* @param {String} [options.wantArray=true] If true the output is sorted into an array of values, if false an object (with each key as the module ID) is returned
	...
	*/
	exec(options) {
		...
		return Promise.resolve()
				...
				.map(module => Promise.resolve()
					...
					.then(cacheValue => {
						...
						return domain.run(()=> Promise.resolve()
							.then(()=> module._handler.call(module, this))
							.then(result => {
			...
			// }}}
			// Sort into an array (if settings.wantArray) {{{
			.then(()=> {
				if (!settings.wantArray) return; // Keep as lookup object

				report.modules = sortBy(Object.values(report.modules), m =>
					settings.sort.map(s =>
						/-/.test(s) || /^[A-Z]+$/.test(s) ? s.split(/-/).map(status => // Either all uppercase or seperated by hyphens
							m.status == status ? 'A' : 'Z'
						).join('-')
						: m[s] // Use key lookup
					)
